...
class BBOAgent(Agent):
    ...
    def results_pi_update_with_explore(self, explore):
        ...
        if self.algorithm_method in ['value', 'anchor']:
            val = self.output_denorm(self.value_net(self.pi_net.pi.detach()).detach().cpu()).item()
            key = 'value'

        self.results['best_observed'].extend([best_observed]*explore)
        self.results['reward_pi_evaluate'].extend([pi_eval]*explore)
        self.results['best_pi_evaluate'].extend([best_pi_evaluate]*explore)
        self.results['policies'].extend([pi]*explore)
        self.results['grad'].extend([grad]*explore)
        self.results['dist_x'].extend([dist_x]*explore)
        self.results['in_trust'].extend([True]*explore)
        ...
        self.results['r_norm_mean'].extend([self.mean]*explore)
        self.results['r_norm_sigma'].extend([self.std]*explore)
        self.results[key].extend([val]*explore)
    ...
    def value_optimize(self, value_iter):
        ...
        self.calc_clip_and_scaler(self.tensor_replay_reward[-self.batch:])
        self.tensor_replay_reward_norm = self.norm(self.tensor_replay_reward).to(self.device)
